...
# ---------- PII masking for structured payloads ----------
def mask_pii(text, known_names=None):
    """
    Mask PII across a WHOLE serialized payload.

    WHY THE KEY-BASED APPROACH WAS NOT ENOUGH (a real leak found in adversarial testing):
    masking only values under name-like KEYS ("AccusedName": "...") missed every name embedded in
    GENERATED PROSE — e.g. the timeline event  "what": "Same person (Ramaiah K) in FIR 18",
    or a risk explanation "Accused 'Ramesh Gowda' is a repeat offender". Our system produces a lot
    of narrative text, so a role forbidden from seeing PII was still reading real names.

    So we mask by VALUE as well as by key: any known person name from the database is redacted
    wherever it appears, including inside sentences. Longest names first, so a name that contains
    another name is not partially replaced.
    """
    import re as _re
    # 1. phone numbers anywhere
    text = _re.sub(r'\+91\d{10}', '[PHONE-MASKED]', text)
    # 2. values under name-like keys
    text = _re.sub(r'"((?:[A-Za-z]*)[Nn]ame)"\s*:\s*"[^"]*"', r'"\1": "[NAME-MASKED]"', text)
    # 3. THE FIX: known person names ANYWHERE in the payload, including inside prose
    if known_names:
        for nm in sorted({n for n in known_names if n and len(str(n)) > 2},
                         key=lambda s: -len(str(s))):
            text = text.replace(str(nm), "[NAME-MASKED]")
    return text


def collect_person_names(store):
    """Every person name in the corpus — the vocabulary the masker must redact."""
    names = set()
    for a in store.all_accused():
        if a.get("AccusedName"):
            names.add(a["AccusedName"])
    for c in store.get_all_cases():
        for v in store.get_victims_for_case(c["CaseMasterID"]):
            if v.get("VictimName"):
                names.add(v["VictimName"])
        for cp in store.get_complainants_for_case(c["CaseMasterID"]):
            if cp.get("ComplainantName"):
                names.add(cp["ComplainantName"])
    return names
