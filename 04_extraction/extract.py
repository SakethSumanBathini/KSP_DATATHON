...
# UPI VPA (Virtual Payment Address). Format: identifier@psp
# BUG FIX: the previous pattern hardcoded only 4 PSP handles (okaxis|oksbi|okhdfc|paytm), so it
# SILENTLY MISSED every other provider (okicici, ybl, upi, ibl, axl, ...). Our "100% extraction"
# figure was an artefact: the generator emitted only the 4 PSPs the regex already knew.
# Real UPI has dozens of PSPs, so we match the FORMAT, not a whitelist.
#
# Distinguishing UPI from an EMAIL address: a UPI PSP handle has NO dot (no TLD).
# foo@oksbi      -> UPI     (no dot after @)
# foo@gmail.com  -> EMAIL   (dot after @)  -> excluded by the negative lookahead
# The lookahead must reject an EMAIL TLD (dot IMMEDIATELY followed by letters, e.g. '.com')
# but ACCEPT a sentence-ending period (dot followed by a space/EOL). A naive (?!\.) rejected
# every handle that ended a sentence — it passed when the handle was tested in isolation and
# failed silently in real prose. This is why we test against real BriefFacts, not toy strings.
RE_UPI = re.compile(r'\b([a-z0-9][a-z0-9._-]{1,})@([a-z][a-z0-9]{1,19})\b(?!\.[a-z])', re.IGNORECASE)

# Known PSPs — used to mark confidence, NOT to gate matching.
KNOWN_PSPS = {"oksbi","okhdfc","okicici","okaxis","paytm","ybl","upi","ibl","axl","apl",
              "okbizaxis","yesg","hdfcbank","sbi","icici","axisbank","freecharge","airtel"}
